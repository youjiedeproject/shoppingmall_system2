...
<div id="myc2">
    <ul>
        ...
        <li><a href="${pageContext.request.contextPath }/product/categoryProduct?cid=${cat.cid}">${cat.cname}</a><li>
        </c:forEach>
...
<div style="text-align:center">
    <ul id="pageUl">
        <c:if test="${pageBean.currentPage!=1 }">
            <li><a href="${pageContext.request.contextPath }/product/paging?currentPage=${pageBean.currentPage-1}&cid=${cid}">上一页</a></li>
        </c:if>
        <c:if test="${pageBean.currentPage==1 }">
            <li><a>上一页</a></li>
        </c:if>
        <c:forEach begin="1" end="${pageBean.totalPage }" var="page">
            <c:if test="${page!=pageBean.currentPage }">
                <li><a href="${pageContext.request.contextPath }/product/paging?currentPage=${page }&cid=${cid}">${page }</a></li>
            </c:if>
            <c:if test="${page==pageBean.currentPage }">
        ...
        <c:if test="${pageBean.currentPage!=pageBean.totalPage }">
            <li><a href="${pageContext.request.contextPath }/product/paging?currentPage=${pageBean.currentPage+1}&cid=${cid}">下一页</a></li>
        </c:if>
        <c:if test="${pageBean.currentPage==pageBean.totalPage }">
            <li>下一页</li>
        </c:if>
